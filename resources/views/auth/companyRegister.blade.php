...
<body class="bg-gradient-to-b from-[#0052A2] to-[#0274D3] text-white min-h-screen py-12">
    ...
    <div class="max-w-md mx-auto bg-white/10 backdrop-blur-xl border border-white/20 
        ...
        <h1 class="text-center text-3xl font-bold mb-6">Company Register </h1>
        <h3 class="text-center text-3xl font-bold mb-6">Create Your Account</h3>

        {{-- Registration Form --}}
        <form method="POST" action="{{ route('web.register.company') }}">
            @csrf
            ...
            <div x-show="role === 'company'" x-transition>
                <label class="block mb-2 font-semibold">Company Address</label>
                <textarea name="address" rows="3"
                    class="w-full px-4 py-2 rounded-lg bg-white/20 border border-white/30
                    text-white placeholder-white/60 mb-6"></textarea>
            </div>
